...
def list_courses(
    ...
    for course in courses:
        ...
        result.append({
            ...
            "instructor_profile_image_url": instructor.profile_image_url if instructor else None,
...
def generate_summary(
    ...
    if transcript_text:
        ...
        if course_category:
            ...
            if any(kw in category_lower for kw in ["수학", "math", "mathematics"]):
                subject_type = "수학"
                subject_guidance = "- 수식과 공식은 명확하게 표현하고, 단계별 풀이 과정을 표로 정리하세요.\n- 개념 간의 관계를 표로 비교 정리하세요 (예: 함수의 종류, 정리와 공식).\n"
            elif any(kw in category_lower for kw in ["영어", "english", "영문"]):
                subject_type = "영어"
                subject_guidance = "- 문법 규칙, 단어/표현, 예문을 표로 정리하세요.\n- 문법 포인트와 예시를 비교 표로 작성하세요.\n"
            elif any(kw in category_lower for kw in ["과학", "science", "물리", "화학", "생물"]):
                subject_type = "과학"
                subject_guidance = "- 실험 과정, 원리, 현상을 표로 정리하세요.\n- 개념 비교, 실험 조건과 결과를 표로 작성하세요.\n"
            elif any(kw in category_lower for kw in ["역사", "history", "사회"]):
                subject_type = "역사/사회"
                subject_guidance = "- 시대별, 사건별 정보를 표로 정리하세요.\n- 인물, 사건, 연대를 표로 비교하세요.\n"
        ...
        summary_prompt = (
            ...
            f"- **굵은 글씨**로 핵심 키워드 강조\n"
            f"- 명확하고 간결한 문장 사용\n"
            f"- 이 섹션에서는 전체적인 흐름과 목적만 설명하세요\n\n"
            f"### 2. 핵심 개념 정리 (## 핵심 개념)\n"
            f"- 주요 개념들을 **표 형식**으로 정리 (반드시 표를 사용하세요)\n"
            f"- 과목 특성에 맞는 표 구조 사용:\n"
            f"  - 수학: | 개념/공식 | 설명 | 적용 예시 |\n"
            f"  - 영어: | 문법/표현 | 설명 | 예문 |\n"
            f"  - 과학: | 개념/원리 | 설명 | 실험/관찰 |\n"
            f"  - 기타: | 개념 | 설명 | 예시/비고 |\n"
            f"{subject_guidance}"
            f"- 각 개념을 한 줄씩 표로 작성 (최소 3개 이상의 개념 포함)\n"
            f"- 표는 반드시 헤더와 구분선을 포함하세요\n"
            f"- 예시:\n"
            ...
            f"- 형식: `- **핵심 키워드**: 설명 내용`\n"
            f"- **중요: 이 섹션은 '핵심 개념 정리'와 중복되지 않도록, 개념 설명이 아닌 학습 포인트나 주의사항을 다루세요.**\n\n"
            f"### 4. 학습 체크리스트 (## 학습 체크리스트)\n"
            f"- 학습자가 확인해야 할 내용을 체크리스트 형식으로\n"
            f"- 형식: `- [ ] 확인할 내용`\n"
            f"- 구체적이고 실행 가능한 항목으로 작성하세요\n\n"
            f"**주의사항:**\n"
            f"- 반드시 마크다운 문법을 정확히 사용해주세요\n"
            f"- 표는 반드시 `|` 기호로 구분하고 헤더와 구분선(`|---|---|---|`)을 포함해주세요\n"
            f"- 섹션은 `##` (H2)로 시작하고, 하위 섹션은 `###` (H3)를 사용해주세요\n"
            f"- 강조는 `**텍스트**` 형식을 사용해주세요\n"
            f"- 불필요한 설명 없이 핵심만 간결하게 작성해주세요\n"
            f"- **매우 중요: 각 섹션(강의 개요, 핵심 개념, 주요 포인트)에 동일한 내용을 절대 반복하지 마세요. 각 섹션은 서로 다른 관점과 정보를 제공해야 합니다.**\n"
            f"  - 강의 개요: 전체 흐름과 목적 (구체적인 개념 나열 금지)\n"
            f"  - 핵심 개념: 구체적인 개념과 정의만 표 형식으로 (학습 포인트나 주의사항 포함 금지)\n"
            f"  - 주요 포인트: 학습 포인트와 주의사항만 불릿 포인트로 (개념 정의 포함 금지)\n"
            f"- 핵심 개념 정리는 반드시 표 형식으로 작성하고, 최소 3개 이상의 개념을 포함하세요.\n"
            f"- 표는 여러 개 사용해도 됩니다. 개념이 많으면 주제별로 표를 나누어 작성하세요.\n"
            ...
            try:
                resp = client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": f"당신은 {subject_type} 교육 전문가이자 학습 자료 작성 전문가입니다. 강의 내용을 분석하여 시각적으로 잘 정리된 마크다운 형식의 요약노트를 작성합니다. 표, 섹션 구분, 강조를 적절히 활용하여 학습자가 쉽게 이해하고 복습할 수 있도록 구조화된 요약을 제공합니다. 각 섹션은 서로 다른 내용을 다루며, 중복을 절대 피합니다."
                        },
...
def generate_quiz(
    ...
    if transcript_text:
        ...
        if course_category:
            ...
            if any(kw in category_lower for kw in ["수학", "math", "mathematics"]):
                subject_type = "수학"
                quiz_guidance = "- 계산 문제, 개념 이해 문제, 공식 적용 문제를 다양하게 포함하세요.\n- 수식이나 공식이 포함된 문제는 명확하게 표현하세요.\n- 단계별 풀이가 필요한 문제도 포함하세요.\n"
            elif any(kw in category_lower for kw in ["영어", "english", "영문"]):
                subject_type = "영어"
                quiz_guidance = "- 문법 문제, 어휘 문제, 독해 문제를 다양하게 포함하세요.\n- 문맥을 이해하는 문제를 포함하세요.\n- 실제 사용 예시를 활용한 문제를 만드세요.\n"
            elif any(kw in category_lower for kw in ["과학", "science", "물리", "화학", "생물"]):
                subject_type = "과학"
                quiz_guidance = "- 개념 이해 문제, 실험 원리 문제, 현상 설명 문제를 포함하세요.\n- 실험 조건과 결과를 묻는 문제를 포함하세요.\n- 원인과 결과를 연결하는 문제를 만드세요.\n"
            elif any(kw in category_lower for kw in ["역사", "history", "사회"]):
                subject_type = "역사/사회"
                quiz_guidance = "- 시대별, 사건별 문제를 다양하게 포함하세요.\n- 인물, 사건, 연대를 묻는 문제를 포함하세요.\n- 원인과 결과를 연결하는 문제를 만드세요.\n"
        ...
        quiz_prompt = (
            ...
            f"## 퀴즈 작성 지침:\n\n"
            f"{quiz_guidance}"
            f"- **중요: 각 문제는 서로 다른 내용을 다뤄야 합니다. 동일한 개념이나 내용을 반복해서 묻지 마세요.**\n"
            f"- 문제 난이도는 초급부터 중급까지 다양하게 포함하세요.\n"
            f"- 선택지는 모두 그럴듯하게 작성하고, 명확한 오답도 포함하세요.\n"
            f"- 정답은 강의 내용에 명확히 나온 내용이어야 합니다.\n"
            f"- 문제는 강의의 핵심 내용을 골고루 다루도록 하세요.\n\n"
            ...
            f"D. [선택지4]\n"
            f"정답: A (또는 B, C, D)\n\n"
            f"이런 형식으로 {num_questions}문제 만들어주세요. 각 문제는 서로 다른 내용을 다뤄야 합니다."
        )
            ...
            try:
                resp = client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": f"당신은 {subject_type} 교육 전문가입니다. 강의 내용을 바탕으로 과목 특성에 맞는 적절한 난이도의 객관식 퀴즈를 만듭니다. 각 문제는 서로 다른 내용을 다루고, 강의의 핵심 내용을 골고루 반영합니다."
                        },
                        {"role": "user", "content": quiz_prompt}
                    ],
                    temperature=0.4,  # 일관성과 다양성의 균형
                )
                answer = resp.choices[0].message.content
            except Exception as e:
